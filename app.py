...
def index():
    ...
    if credentials == False:
        ...
        all_files = fetch("mimeType = 'application/vnd.google.colaboratory'", sort='modifiedTime desc')
        
        #all_files = fetch("'root' in parents and mimeType = 'application/vnd.google-apps.folder'", sort='modifiedTime desc')
        #s = ""
        #for file in all_files:
        #    s += "%s, %s, %s <a href=""https://colab.research.google.com/drive/%s"">%s</a> <br>" % (file['name'],file['id'],file['mimeType'], file['id'],file['name'], )
            #https://colab.research.google.com/drive/13Hw-4ugS2WsbeJuOypsxn2DBVQM1Z_tZ
...
def fetch(query, sort='modifiedTime desc'):
    ...
    results = service.files().list(
        q=query,orderBy=sort,pageSize=10,fields="nextPageToken, files(id, name, mimeType, modifiedTime)").execute()
